...
| `STAMP_AUDIT_FAIL_CLOSED` | Deny once the check-path audit buffer has **detected** that it cannot write to the chain. On by default. Read the guarantee narrowly: the buffer is asynchronous, so it detects on a failed flush, and the judgments served in the flush interval before that are answered and then counted into a gap marker rather than refused — measured at 276–364 allows over 769–826 ms with the shipped one-second `STAMP_AUDIT_FLUSH_INTERVAL`. It is not "no allow is served that is not in the chain"; closing that window means a synchronous chain append on every check, which is the trade R32 made in the other direction. See [failure modes](docs/operations/failure-modes.md#the-check-surface-serves-allows-for-one-audit-flush-interval). |
...
| `STAMP_CHALLENGE_ISSUE_RATE_PER_SECOND`, `STAMP_CHALLENGE_ISSUE_RATE_BURST` | How often **one caller** may have a challenge opened against **one subject**. This is not the re-issue interval: that one is keyed on the decision content and so suppresses a re-evaluation of one decision, while a caller creating a hundred *different* decisions for one person is a hundred different keys and a hundred prompts on one phone. Defaults per handler — a step-up prompt to a human at 3 a minute bursting to 5, a webhook to a machine at 1 a second bursting to 10. It was keyed on the subject alone until #40's follow-up, and that made it aimable: a subject id is a `sub` claim or an account number, so anyone who could name a person could hold their bucket empty and have every authorization they needed come back denied. The webhook handler is deliberately still keyed on the subject alone — what a refusal there protects is somebody else's system, whose defence is that the *total* is bounded rather than each caller's share. **Per instance**, like the decide rates. A negative rate removes the limit. |
| `STAMP_CHALLENGE_ISSUE_SUBJECT_CEILING_PER_SECOND`, `STAMP_CHALLENGE_ISSUE_SUBJECT_CEILING_BURST` | The step-up ceiling above that: how many prompts one person may be sent in total, across every caller. Defaults to 20 an hour bursting to 10 — one prompt every three minutes sustained is already where people stop reading them and start clearing them, which is the state MFA-fatigue attacks are trying to produce. **The burst must be larger than one caller's**, and the process refuses to start otherwise: a ceiling one caller can empty in an instant is the shared bucket again. Over either budget nothing reaches the IdP, the decide answers a denied decision with reason `challenge_rate_limited` and a `Retry-After`, and **no decision row is written** — so a person whose budget somebody else is spending does not accumulate refusals on their own history. A negative rate removes the ceiling; turning off one of the two budgets does not turn off the other. |
| `STAMP_APPROVAL_RATE_PER_SECOND`, `STAMP_APPROVAL_RATE_BURST` | How often one approver may submit an approval. Defaults to 2 a second, bursting to 20. Over the limit the console gets `429 rate_limited` — a status code here, unlike the two above, because there is a console request to answer — and the refusal is audited with reason `approval_rate_limited`, which is its own ground and not the decide path's `rate_limited`. **Per instance**. A negative rate removes the limit. |
| `STAMP_CANCELLATION_RATE_PER_SECOND`, `STAMP_CANCELLATION_RATE_BURST` | How often one authority may attempt to cancel a delay. Defaults to 1 a second, bursting to 5 — tighter than the approval budget above, because an approver works through a queue while a cancellation is one person stopping one wait once, and what the burst has to cover is a double-clicked button and a reloaded page rather than work. Over the limit the console gets `429 rate_limited` with a `Retry-After`, and the refusal is audited with reason `cancellation_rate_limited` — its own ground, so a reader of the chain can tell which of the five surfaces shed. This one is not a budget on the cost of a cancellation that succeeds: it is a budget on the cost of one that is *refused*, because a caller with no standing on a decision that exists makes the lifecycle write a synchronous audit-chain append, on the serialized write path, for the whole life of that decision. **Per instance**. A negative rate removes the limit. |
