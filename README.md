...
| `STAMP_CHALLENGE_ISSUE_RATE_PER_SECOND`, `STAMP_CHALLENGE_ISSUE_RATE_BURST` | How often one subject may have a challenge opened against them. This is not the re-issue interval: that one is keyed on the decision content and so suppresses a re-evaluation of one decision, while a caller creating a hundred *different* decisions for one person is a hundred different keys and a hundred prompts on one phone. Defaults per handler — a step-up prompt to a human at 3 a minute bursting to 5, a webhook to a machine at 1 a second bursting to 10. Over the limit, the challenge is opened failed and the decision denies on its own ground — `challenge_rate_limited`, distinct from the `challenge_failed` a rejected prompt produces, so a shed request and a refused one do not read alike. The step-up records `issue_rate_limited` on the challenge row and the webhook records `rate_limited`, and neither reaches the IdP or the target. **Per instance**, like the decide rates. A negative rate removes the limit. |
