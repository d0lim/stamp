...
그러면 무엇을 얻는가. **관측 가능성이다.** 메시 사이드카·로그 파이프라인·메트릭 익스포터는 본문을 해석하지 않고 헤더 표를 읽으므로, 이제 셰딩된 요청과 판정된 요청을 세어 가를 수 있다 — [#45](https://github.com/d0lim/stamp/issues/45)가 실제로 가져다준 것이 이것이고, "PEP가 이 헤더를 보고 물러난다"가 아니다. 반대로 **승인 제출의 `429`에서는 명세 그대로 동작하고 클라이언트들도 실제로 지킨다.** 두 자리를 같은 문장으로 설명하면 하나는 과장이 된다.
