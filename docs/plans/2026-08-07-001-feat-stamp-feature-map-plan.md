...
- 아래에서 위로 병합하되 **GitHub의 base 자동 재지정에 기대지 않는다.** 재지정은 head 브랜치 삭제를 조건으로 하고, 성립하더라도 base 포인터만 옮길 뿐 자식 히스토리를 다시 쓰지 않는다 — squash 병합에서는 부모의 커밋이 main에 그대로 남지 않으므로 자식 PR이 부모의 diff를 통째로 다시 표시하고 병합 시 충돌한다. base가 병합될 때마다 자식을 `git rebase --onto main <옛-base> <자식-브랜치>`로 새 main 위에 옮기고 force push한다. 이 절차는 병합 방식과 무관하게 성립하므로 자동 재지정 여부를 따질 필요가 없다.
