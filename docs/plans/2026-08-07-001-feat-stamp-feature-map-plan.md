...
- ~~감사 콘솔의 조회 축(기간·정책·주체·상태)과 정렬·페이지네이션, 대응 인덱스 (U16 설계 시, U4 마이그레이션에 반영).~~ **U16에서 확정.** 축 넷은 `created_at`의 반열린 구간 `[from, to)`, `policy_id`·`subject_id`·`state`의 정확 일치다. 정렬은 `(created_at, id)` 내림차순 하나이며 선택 불가 — 감사 읽기는 타임라인이고, 전순서가 하나여야 커서가 성립한다. 페이지네이션은 OFFSET이 아니라 `(created_at, id)` 키셋 커서다(추가 삽입이 뒤 페이지를 밀어 행을 누락시키지 않는다). 인덱스는 마이그레이션 `000007_decision_history`가 축별로 `(축, created_at DESC, id DESC)` 넷과 승인함 후보 조인용 부분 인덱스 하나를 만든다. 근거는 `internal/store/history.go`와 그 마이그레이션 주석에 있다.
- PEP 인증 방식을 mTLS로 고정할지 client_credentials로 통일할지 — 데모 번들이 보여줄 기본값과 함께 정한다 (U5).
- ~~벤치의 경고에서 실패로의 승격 시점 — 첫 수치가 나온 뒤 확정한다 (U17).~~ **U17에서 조건을 확정했고 시점은 U18에 남는다.** 승격의 전제는 날짜가 아니라 **그 임계값을 그 러너에서 측정했는가**다: 임계값은 러너별 프로파일에 속하고(`bench/thresholds.json`), 프로파일이 `calibrated: false`인 동안 벤치는 수치와 직전 대비 회귀만 보고하며 절대 판정을 내지 않는다. 보정된 프로파일의 임계값은 같은 커밋 10반복의 최악값에 지연 ×1.5·처리량 ×0.6의 여유를 둔 값이고, 회귀 허용치는 측정된 실행 간 드리프트의 3배 이상이어야 한다 — 러너 변동보다 좁은 게이트는 소음이기 때문이다. 로컬 프로파일은 U17이 보정했고(근거는 프로파일의 `basis` 문자열마다), **CI 프로파일은 미보정이다**. 그것을 채우는 것이 승격의 남은 선행 조건이다 (U18).
