...
계획서에 착지 방식이 없다는 결핍을 메웠다. 구현 유닛을 그대로 PR 단위로 쓰고, 의존 그래프의 합류점에서 스택을 끊는 티어 구조를 확정했다(D25). PR 본문의 네 항목(배경·해결법·근거·집중해서 봐야할 것)도 계획서의 Landing 전략에 고정했다 — 특히 근거 항목이 이 결정 로그를 인용만 하고 다시 논증하지 않게 하는 규칙이 함께 들어갔다.
